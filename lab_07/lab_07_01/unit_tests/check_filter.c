...
static int unsorted_filter[] = {9, 4, 5, 6, 2, 8, 0};
...
START_TEST(unit_key)
    {
        ...
        calc_limits(sorted, sorted + 10, &maximum, &minimum);
        ck_assert(&sorted[0] == maximum);
        ck_assert(&sorted[9] == minimum);
        calc_limits(unsorted, unsorted + 10, &maximum, &minimum);
        ck_assert(&unsorted[3] == maximum);
        ck_assert(&unsorted[9] == minimum);
    }
END_TEST
...
Suite *main_suite() {
    ...
    tcase_add_test(core_case, unit_key);
